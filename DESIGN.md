...
tools:
  survey:
    enabled: true
    languages: [rust, typescript, javascript, python, go, csharp]  # see D55, extended by D58
...
| D55 | **Survey v1 language scope and pillar wiring.** Language scope: Rust, TypeScript/JavaScript, Python, Go (extended to C# by D58). Symbol index + FTS5 full-text search + caller/callee/impact. Framework-aware routing and iOS/RN cross-language bridging deferred. Pillar wiring: Tokens — a `derrick gain` line for survey (`survey_tokens_saved`), owned by the token-economist role so numbers reconcile; Memory — `derrick init` seeds the index location as a reference memory entry; Parallelism — per-file parse fan-out at build time; index DB shared read-only across worktree runs per D38, behind a reader pool (`tools.survey.reader_pool`). Config knob at §9.B.8. | §9.B.8 / §9.A.1 / §9.C |
