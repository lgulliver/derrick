...
**One narrowing (D66)**: model selection is the single dimension
derrick now asserts on the run path. When `derrick.yaml` binds a
role to a model, `execute_role_step` resolves that model and sets
`HostRequest.model`; the host adapter forwards it as a normalised
`--model` flag. When no model is configured the host keeps its own
default, preserving the no-configuration-needed path. Everything
else — context, agents, hooks, sub-agent spawn — remains the
host's domain. The cloud Copilot crew hand is explicitly excluded
from model forwarding (the model is server-side; there is no local
CLI to receive `--model`). See D66 for the full scope.
